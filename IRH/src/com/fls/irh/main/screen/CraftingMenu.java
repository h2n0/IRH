...
import com.fls.irh.main.crafting.Recipe;
...
public class CraftingMenu extends Menu {
	...
	public void render(Screen screen) {
		...
		if (recipes.size() > 0) {
			...
			Font.draw("" + hasResultItems, screen, xo + 8, 2 * 8, Colour.get(-1, 555, 555, 555));
			...
			for (int i = 0; i < costs.size(); i++) {
				...
				int color = Colour.get(-1, 555, 555, 555);
				if (has < requiredAmt) {
					color = Colour.get(-1, 222, 222, 222);
				}
				if (has > 99) has = 99;
