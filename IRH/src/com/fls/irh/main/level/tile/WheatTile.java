...
public class WheatTile extends Tile {
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		int col = Colour.get(level.dirtColor - 121, level.dirtColor - 11, level.dirtColor, 50);
		int icon = age / 10;
		if (icon >= 3) {
			col = Colour.get(level.dirtColor - 121, level.dirtColor - 11, 50 + (icon) * 100, 40 + (icon - 3) * 2 * 100);
			if (age == 50) {
				col = Colour.get(0, 0, 50 + (icon) * 100, 40 + (icon - 3) * 2 * 100);
			}
			icon = 3;
		}
